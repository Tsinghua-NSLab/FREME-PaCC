...
{
    state_t major_trans[_size];
    state_t major_num[_size];
    state_t default_trans[_size];
    state_t default_num[_size];
    state_t group_major_trans, group_major_num;
    state_t major[C] = {0}, count[C] = {0};
    label_t be_done[_size];
    state_t ds, s, t, tmp_t;
    unsigned long trans_stats = 0;
    int i, j, k, tmp_i;
    ...
    for (s=0; s<_size; s++) {
        ...
        for (i=0; i<C; i++) {
            for (j=0; j<k; j++) {
                if (state_table[s][i] == major[j]) {
                    break;
                }
            }
            count[j]++;
            if (j == k) {
                major[j] = state_table[s][i];
                k++;
            }
        }
        i = 0;
    ...
    for (s=0; s<_size; ) {
        ...
        group_major_trans = major_trans[s];
        group_major_num = 0;
        for (t=s; t<_size; t++) {
            if (be_done[t]) {
                continue;
            }
            if (major_trans[t] == group_major_trans && major_num[t] > group_major_num) {
                ds = t;
                default_trans[s] = ds;
                group_major_num = major_num[t];
                be_done[ds] = 1;
            }
        }
        for (t=s; t<_size; t++) {
            if (be_done[t] || major_trans[t] != group_major_trans) {
                continue;
            }
            for (i=0; i<C; i++) {
                if (state_table[t][i] == state_table[ds][i]) {
                    default_num[t]++;
                }
            }
            if (default_num[t] > major_num[t]) {
                major_num[t] = 0;
                be_done[t] = 1;
            } else {
                default_num[t] = 0;
    ...
    for (s=0; s<_size; s++) {
        trans_stats += 1 + (C - major_num[s] - default_num[s]);
    }
    ...
    for (s=0; s<_size; s++) {
        ...
        tmp_t = G;
        for (i=0; i<C; i++) {
            if (state_table[s][i] == major_trans[s]) {
                continue;
            }
            if (t != G && state_table[s][i] == state_table[t][i]) {
                continue;
            }
            if (state_table[s][i] == tmp_t && i == (tmp_i + 1)) {
                trans_stats--;
            }
            tmp_i = i;
            tmp_t = state_table[s][i];
        }
    }

    printf(">> compression ratio = %f\n", (256*_size - trans_stats)*1.0/(256*_size));
/*
    char        cmd[C];
    FILE        *fp;
...
static int load_regex_data(datum_unit_t *du)
{
    ...
    for (t=0; t<du->nt; t++) {
        ...
        for (s=0; s<du->ns[t]; s++) {
            ...
            trans_num += 256;
            MFA[t][s].mtrans = (mtrans_t *)malloc((MFA[t][s].num + 1) * sizeof(mtrans_t));
            if (!MFA[t][s].mtrans) {
                rv = -17;
                goto err;
            }
            memset(MFA[t][s].mtrans, 0, (MFA[t][s].num + 1) * sizeof(mtrans_t));
            j = 0;
            for (i=0; i<C; i++) {
...
static int process(datum_unit_t *du)
{
    ...
    while (n < ni) {
        ...
        do {
            ...
            //}
            //mem_times++;
            if ((sw->table == 0 && sw->state == 0) || (sw->table == 1 && sw->state == 1)) {
                sw->state = MFA[sw->table][sw->state].mtrans[c].state;
            } else {
